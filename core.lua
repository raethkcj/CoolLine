...
do -- cache spells that have a cooldown
    ...
    local function CacheBook(btype)
        ...
        elseif IS_WOW_CLASSIC then
            -- luacheck: ignore offsets
            local numTabs = GetNumSpellTabs()
            for i = 1, numTabs do
                local _, _, offset, numSpells = GetSpellTabInfo(i)
                for j = 1, offset + numSpells do
                    -- luacheck: ignore spellRank spellId
...
do -- scans spellbook to update cooldowns, throttled since the event fires a lot
    ...
    local GetSpellCooldown, GetSpellCharges = GetSpellCooldown, GetSpellCharges
