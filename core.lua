...
function ShowOptions(a1)
    ...
    CoolLineDD.initialize = function(self, lvl)
        if lvl == 1 then
            info.isTitle = true
            info.notCheckable = 1
            local already_has_mover = IsAddOnLoaded("ElvUI_CoolLine_Wrapper")
            ...
            -- remove unlock if ElvUI_CoolLine_Wrapper is loaded: it provides a mover
            if already_has_mover then
                info.isTitle = true
                info.notCheckable = 1
                AddButton(lvl, "|cff88ffffNot movable|r |cff88ff88(you have ElvUI_CoolLine_Wrapper addon for that purpose)|r")
            else
